...
#         Jon Carey
#         Ross Peoples <ross.peoples@gmail.com>
####
...
from .cim_obj import cmp, cmpname, NocaseDict, tocimobj, CIMProperty, CIMClass
from .cim_operations import CIMError, CIMInstance, CIMInstanceName, is_subclass
from .cim_types import cimtype
...
def _path_equals_ignore_host(lhs, rhs):
    ...
    return not (cmpname(rhs.classname, lhs.classname) or
                cmp(rhs.keybindings, lhs.keybindings) or
                cmpname(rhs.namespace, lhs.namespace))
...
class CIMProvider(object):
    ...
    def MI_enumInstanceNames(self,
        ...
        _strip_quals(keys)
        path = CIMInstanceName(classname=cimClass.classname,
                                      namespace=ns)
        model = CIMInstance(classname=cimClass.classname,
                                   properties=keys,
                                   path=path)
    ...
    def MI_getInstance(self,
        ...
        if propertyList is not None:
            ...
            props = cimClass.properties
        _strip_quals(props)
        model = CIMInstance(classname=instanceName.classname,
                                   properties=props,
                                   path=instanceName)
        for k, v in instanceName.keybindings.items():
            _type = cimClass.properties[k].type

            if _type != 'reference':
                v = val = tocimobj(_type, v)
            model.__setitem__(k, CIMProperty(name=k, type=_type,
                                                    value=v))
    ...
    def MI_associators(self,
        ...
        _strip_quals(plist)
        model = CIMInstance(classname=assocClass.classname,
                                   properties=plist)
        model.path = CIMInstanceName(classname=assocClass.classname,
        ...
        if hasattr(self, lmethName):
            ...
            def add_type(v, _tp):
                ...
                if type(v) == list and len(v) > 0:
                    lv = v[0]
                if isinstance(lv, CIMClass):
                    tp = 'class'
                elif isinstance(lv, CIMInstance):
                    tp = 'instance'
                elif isinstance(lv, CIMInstanceName):
                    tp = 'reference'
...
def filter_instance(inst, plist):
    ...
    if plist is not None:
        for pname in list(inst.properties.keys()):
    ...
    for _ in obj:
        if _ not in inst.properties:
            raise CIMError(CIM_ERR_FAILED,
                                  ...
                                  %(inst.classname, _))
        keys[_] = inst[_]
    inst.path = CIMInstanceName(classname=inst.classname,
                                       keybindings=keys,
...
def _strip_quals(props):
    for prop in props.values(): # remove all but key quals
        try:
            prop.qualifiers = NocaseDict({'KEY':
                                                 prop.qualifiers['KEY']})
        except KeyError:
            prop.qualifiers = NocaseDict()
...
def codegen(cc):
    ...
    for obj in list(cc.properties.values()) + list(cc.methods.values()):
        if 'valuemap' in obj.qualifiers:
...
class %(classname)sProvider(pywbem.CIMProvider):
    ...
    for kp in keyProps:
        ...
        #   model['%s']''' % kp.name
    code += '\n'
    props = list(cc.properties.values())
    props.sort()
    for prop in props:
        if 'key' in prop.qualifiers:
            continue
