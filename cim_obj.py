...
class CIMInstanceName(object):
    ...
    def has_key(self, key): return ley in self.keybindings
    ...
    def iterkeys(self): return self.keybindings.keys()
    def itervalues(self): return self.keybindings.values()
    def iteritems(self): return self.keybindings.items()
    ...
    def tocimxml(self):
        ...
        elif isinstance(self.keybindings, six.integer_types + (float,)):
            ...
            for kb in self.keybindings.items():
                ...
                if type(kb[1]) == bool:
                    ...
                    if kb[1]:
                        ...
                        value = 'FALSE'
                elif isinstance(kb[1], six.integer_types + (float,)):
                    # pywbem.cim_type.{Sint32, Real64, ... } derive from
                    # long or float
                    _type = 'numeric'
                    value = str(kb[1])
                elif isinstance(kb[1], six.string_types):
                    _type = 'string'
                    value = kb[1].decode('utf-8') if isinstance(kb[1], six.binary_type) \
                                                  else kb[1]
                else:
                    raise TypeError('Invalid keybinding type for keybinding '\
                            '%s: %s' % (kb[0], str(type(kb[1]))))
...
class CIMMethod(object):
    ...
    def tomof(self):
        ...
        s += '%s(%s);' % \
             (self.name,
              ', '.join([p.tomof() for p in self.parameters.values()]))
...
class CIMParameter(object):
    ...
    def __init__(self, name, _type, reference_class=None, is_array=None,
...
class CIMQualifier(object):
    ...
    def __init__(self, name, value, _type=None, propagated=None,
...
def tocimxml(value):
    ...
    if isinstance(value, (cim_types.CIMType, int) + six.string_types):
...
def tocimobj(_type, value):
    ...
    if _type != 'string' and isinstance(value, six.string_types) and not value:
        return None

    # Lists of values

    if type(value) == list:
        return [tocimobj(_type, x) for x in value]
