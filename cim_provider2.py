...
from .cim_obj import cmp, cmpname, CIMInstance, CIMInstanceName, CIMParameter, CIMClass, NocaseDict
...
class CIMProvider2(object):
    ...
    def MI_enumInstanceNames(self,
        ...
        model = CIMInstance(classname=objPath.classname,
                                   path=objPath)
    ...
    def MI_associators(self,
        ...
        ch = env.get_cimom_handle()
        model = CIMInstance(classname=assocClassName)
        model.path = CIMInstanceName(classname=assocClassName,
    ...
    def MI_references(self,
        ...
        if propertyList is not None:
            plist = [s.lower() for s in propertyList]
        model = CIMInstance(classname=resultClassName,
                                   property_list=plist)
        model.path = CIMInstanceName(classname=resultClassName,
                                            namespace=objectName.namespace)
        if role:
    ...
    def MI_invokeMethod(self, env, objectName, methodName, inputParams):
        ...
        if hasattr(self, lmethName):
            ...
            try:
                ...
                raise CIMError(CIM_ERR_INVALID_PARAMETER, str(e))

            def add_type(v):
                if isinstance(v, CIMParameter):
...
##############################################################################
    def MI_associators(self,
        ...
        if cname:
            ...
            lcnames = list(self.provregs.keys())
